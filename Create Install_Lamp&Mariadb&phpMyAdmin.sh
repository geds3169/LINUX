...
# If you want to help me improve it write me
#################################################
...
echo -n "Enter name of the desired database CMS, exemple: wordpress"
read baseCMS
...
# Delete database and user if exist and create again ( maybe can i change it to "if / else / do"
################################################################################################
set -e
...
DROP DATABASE IF EXISTS $baseCMS;
CREATE DATABASE $database;
CREATE USER '$root'@'localhost' IDENTIFIED BY '$password';
GRANT ALL PRIVILEGES ON $database.* TO '$root'@'localhost' IDENTIFIED BY '$password';
CREATE DATABASE $baseCMS;
CREATE USER '$user'@'localhost' IDENTIFIED BY '$pass';
GRANT ALL PRIVILEGES ON $baseCMS.* TO '$user'@'localhost' IDENTIFIED BY '$pass';
FLUSH PRIVILEGES;
EOF

# Return some informations
##########################
echo -e "$Red \n remember: the login is root and the root password to administer phpmyadmin $Color_Off"
echo -e "$Red \n remember: the login $user and password: $pass for administer a new database CMS: $database $Color_Off"
sleep 1
...
# Clean Folder where is downloaded the phpmyadmin package
############################################################
echo -e "$Cyan \n Clean folder removing zip downloaded $Color_Off"
rm -R -f  phpmyadmin
rm -f phpmyadmin.zip

# Configure blowfishsecret tokens for phpmyadmin
#################################################
...
# Verifying install some package used by the CMS
#################################################
...
# TWEAKS and Settings and permissions
#####################################
...
# Enabling Mod Rewrite, required for WordPress permalinks and create.htaccess files & htpasswd
##############################################################################################
...
# Write configuration apache phpmyadmin
